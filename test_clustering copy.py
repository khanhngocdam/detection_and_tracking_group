...
# Khởi tạo Deep SORT
deepsort = DeepSort(
    model_path="../deep_sort_pytorch/deep_sort/deep/checkpoint/ckpt.t7",  # Đường dẫn đến file trọng số
...
input_folder = "./v1"
output_folder = "output_frames_cluster_v1"
...
for idx, filename in enumerate(sorted(os.listdir(input_folder))):
    if filename.lower().endswith(('.jpg', '.jpeg', '.png')):
        ...
        if len(centers) >= 2:
            ...
            # Vẽ đường nối giữa các điểm trong cùng một nhóm
            for group_id, members in current_groups.items():
                if len(members) >= 2:
                    ...
                    # Vẽ đường kết nối giữa các thành viên trong nhóm
                    for i in range(len(member_indices)):
                        for j in range(i+1, len(member_indices)):
                            idx1, idx2 = member_indices[i], member_indices[j]
                            pt1 = (int(centers[idx1][0]), int(centers[idx1][1]))
                            pt2 = (int(centers[idx2][0]), int(centers[idx2][1]))
                            cv2.line(image_with_clusters, pt1, pt2, color, 2)
        else:
